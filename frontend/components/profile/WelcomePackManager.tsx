...
export const WelcomePackManager: FC<WelcomePackManagerProps> = ({ api, allResources, userRoles }) => {
  ...
  useEffect(() => {
    const loadConfig = async () => {
      ...
      try {
        ...
        // Handle the API response - your client returns data directly
        const extractedData = response.data || response;
        const { config, message } = extractedData;
  ...
  const handleSave = async () => {
    ...
    setStatus('idle');
    try {
      const configToSend: PackConfig = {};
      for (const role in packConfig) {
        configToSend[role.toLowerCase()] = packConfig[role];
      }
  ...
  const handleAdd = () => {
    ...
    // FIX: Use lowercase version of activeRole to match backend data
    const roleKey = activeRole.toLowerCase();
    const currentPack = packConfig[roleKey] || [];
    ...
    setPackConfig(prev => ({ ...prev, [roleKey]: newPack }));
    setSelectedAvailable([]);
  };
  ...
  const handleMove = (direction: 'up' | 'down') => {
    ...
    setPackConfig(prev => ({ ...prev, [roleKey]: currentPack }));
  };
  ...
  const getDerivedLists = () => {
    ...
    // FIX: Use lowercase version of activeRole to match backend data
    const roleKey = activeRole.toLowerCase();
    const packIds = new Set(packConfig[roleKey] || []);
    const pack = (packConfig[roleKey] || [])
        ...
        {activeRole ? (
          <div className="flex gap-4 h-[450px]">
            <ResourceList 
              ...
              resources={availableList} 
              selected={selectedAvailable} 
              onSelect={(id) => handleSelection('available', id)} 
              disabledIds={currentPackIds} 
            />
            ...
            <div className="flex flex-col justify-center items-center gap-3">
              <ActionButton 
                ...
                ariaLabel="Add selected to pack"
              >
              ...
              <ActionButton 
                ...
                ariaLabel="Remove selected from pack"
              >
                <ChevronLeft size={20} />
              </ActionButton>
            </div>
            ...
            <ResourceList 
              title={`Welcome Pack for "${activeRole}"`} 
              resources={packList} 
              selected={selectedPack} 
              onSelect={(id) => handleSelection('pack', id)} 
            />

            <div className="flex flex-col justify-center items-center gap-3">
              <ActionButton 
                onClick={() => handleMove('up')} 
                disabled={selectedPack.length !== 1} 
          ...
          <div className="flex items-center space-x-4">
            ...
            {status === 'error' && (
              <div className="flex items-center text-red-400">
                <AlertTriangle className="w-4 h-4 mr-2" />Error saving.
              </div>
            )}
          </div>
        </div>
