...
export const InstantNudgeView: FC = () => {
    ...
    const handleAudienceSearch = useCallback(async (query: string) => {
        setIsSearching(true);
        if (!query.trim()) { setFilteredClients(clients); setIsSearching(false); return; }
        try { const results = await api.post('/api/clients/search', { natural_language_query: query }); setFilteredClients(results); } catch (error) { console.error("Failed to search for clients:", error); setFilteredClients([]); } finally { setIsSearching(false); }
    }, [api, clients]);
    ...
    const handleDraftWithAI = async () => {
        if (!topic.trim()) { alert("Please provide a topic or goal for the AI to draft a message."); return; }
        setIsDrafting(true);
        try { const response = await api.post('/api/campaigns/draft-instant-nudge', { topic }); setMessage(response.draft); } catch (error) { console.error("Failed to draft with AI:", error); alert("There was an error generating the AI draft. Please try again."); } finally { setIsDrafting(false); }
    };
            ...
            <section>
                ...
                <div className="p-6 bg-brand-primary border border-white/10 rounded-xl space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-4">
                        <button onClick={handleDraftWithAI} disabled={isDrafting || !topic.trim()} className="flex items-center gap-2 p-3 bg-white/10 rounded-md font-semibold hover:bg-white/20 disabled:opacity-50">{isDrafting ? <Loader2 size={18} className="animate-spin" /> : <Bot size={18} />}{isDrafting ? 'Drafting...' : 'Draft with AI'}</button>
                        <button onClick={handleSendInstantNudge} disabled={isSending || selectedClients.size === 0 || !message.trim()} className="flex items-center gap-2 p-3 bg-primary-action text-brand-dark rounded-md font-semibold hover:brightness-110 disabled:opacity-50">{isSending ? 'Sending...' : <><Send size={18} /> Send to {selectedClients.size} recipients</>}</button>
                    </div>
                </div>
            </section>
        </div>
