...
// --- DEFINITIVE FIX: Removes the duplicated suggestion rendering block. ---
...
export default function ConversationPage({ params }: ConversationPageProps) {
    ...
    useEffect(() => {
        ...
        const pollForUpdates = () => { if (isMounted) fetchConversationData(); };
        pollForUpdates();
    ...
    if (loading && !selectedClient) return <div className="flex-1 flex items-center justify-center text-brand-text-muted">Loading...</div>;
    if (error) return <div className="flex-1 flex items-center justify-center text-red-400">Error: {error}</div>;
    if (!selectedClient) return <div className="flex-1 flex flex-col items-center justify-center h-full text-brand-text-muted p-4"><Users className="w-16 h-16 mb-4" /><h1 className="text-xl font-medium">Client not found</h1></div>;
        ...
        <div className="flex-1 flex min-w-0">
            <main className="flex-1 flex flex-col min-w-0 lg:border-l lg:border-r border-white/10">
                ...
                <div className={clsx("flex flex-col flex-grow min-h-0", activeTab === 'messages' ? 'flex' : 'hidden lg:flex')}>
                   ...
                   {/* --- FIX START --- */}
                   {/* The entire block that was here has been removed. Its functionality */}
                   {/* was moved inside the ChatHistory component to prevent duplication. */}
                   {/* --- FIX END --- */}
