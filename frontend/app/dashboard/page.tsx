            ...
            <div className="bg-brand-dark border border-white/10 rounded-2xl w-full max-w-2xl flex flex-col shadow-2xl">
                ...
                <main className="p-6 space-y-4">
                    <div>
                        ...
                        <input type="date" value={scheduledAt} onChange={e => setScheduledAt(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-sm text-white"/>
                    </div>
                    <div>
                        <label className="text-sm font-semibold text-brand-text-muted mb-2 block">Message Content</label>
                        <textarea value={content} onChange={e => setContent(e.target.value)} rows={6} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-brand-text-main focus:outline-none focus:ring-2 focus:ring-brand-accent"/>
                    </div>
                </main>
...
const ClientIntelCard = ({ client, onUpdate, onReplan }: { client: Client | undefined, onUpdate: (updatedClient: Client) => void, onReplan: () => void }) => {
    ...
    useEffect(() => {
        if (client) { setNotes(client.preferences?.notes?.join('\n') || ''); }
    }, [client]);
                    ...
                    <ul className="space-y-2">
                        {(client.preferences?.notes || []).length > 0 ? (
                            (client.preferences.notes).map((note: string, index: number) => (
                                <li key={index} className="flex items-start gap-3 text-sm"><Info size={14} className="flex-shrink-0 mt-0.5" />{note}</li>
                            ))
                        ) : (
...
export default function DashboardPage() {
  ...
  const handleSendMessage = async () => { /* ... Functionality remains the same ... */ };
  ...
  if (loading) { return (<div className="flex flex-col justify-center items-center h-screen text-brand-text-muted bg-brand-dark"><Sparkles className="w-10 h-10 text-brand-accent animate-spin mb-4" /><p>Loading...</p></div>); }
  if (error) { return (<div className="flex flex-col justify-center items-center h-screen text-red-400 bg-brand-dark"><p>Error loading dashboard</p><p>{error}</p></div>); }
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-[320px_1fr] lg:grid-cols-[320px_1fr_340px] bg-brand-dark text-brand-text-main font-sans">
      <aside className="bg-white/5 border-r border-white/10 p-4 flex flex-col gap-6">
        <div className="flex items-center h-8 mb-4"> <Image src="/AI Nudge Logo.png" alt="AI Nudge Logo" width={260} height={60} priority /> </div>
        ...
        {selectedConversation ? (
          <>
            <header className="flex items-center justify-between p-4 border-b border-white/10 bg-brand-dark/50 backdrop-blur-sm sticky top-0">
              <div className="flex items-center gap-4"> <Avatar name={selectedConversation.client_name} className="w-11 h-11" /> <div><h2 className="text-xl font-bold">{selectedConversation.client_name}</h2><p className="text-sm text-brand-accent">Online</p></div></div>
              <div className="flex items-center gap-2"><button className="p-2 rounded-full text-brand-text-muted hover:bg-white/10"><Phone/></button><button className="p-2 rounded-full text-brand-text-muted hover:bg-white/10"><Video/></button></div>
            </header>
            <div className="flex-grow overflow-y-auto p-6 space-y-6">
              {selectedConversation.messages.map(msg => (
                <div key={msg.id} className={clsx("flex items-end gap-3", msg.sender === 'client' ? 'justify-start' : 'justify-end')}>
                  ...
                  <div className={clsx("p-3 px-4 rounded-t-xl max-w-lg", { 'bg-gray-800 text-brand-text-muted rounded-br-xl': msg.sender === 'client', 'bg-primary-action text-brand-dark font-medium rounded-bl-xl': msg.sender === 'agent' })}><p>{msg.content}</p><p className="text-right text-xs mt-1 opacity-70">{new Date(msg.timestamp).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}</p></div>
                </div>
              ))}
            ...
            <div className="p-4 bg-black/10 border-t border-white/10">
              <div className="relative bg-white/5 border border-white/10 rounded-xl flex items-center">
                ...
                <button className="p-3 text-brand-text-muted hover:text-brand-accent"><Paperclip/></button>
                <button className="bg-primary-action hover:brightness-110 text-brand-dark p-3 rounded-r-xl m-0.5 disabled:opacity-50" onClick={handleSendMessage} disabled={!composerMessage.trim() || isSending}><Send/></button>
              </div>
            </div>
          </>
        ) : (<div className="flex flex-col items-center justify-center h-full text-brand-text-muted"><MessageSquare className="w-16 h-16 mb-4" /><p>Select a conversation</p></div>)}
      </main>
         ...
         {selectedConversation ? (
          <>
            ...
            {scheduledMessages.length > 0 ? (
                ...
                <InfoCard title="Relationship Campaign"><div className="text-center py-4"><p className="text-sm text-brand-text-muted mb-3">No campaign planned.</p><button onClick={() => handlePlanCampaign(selectedConversation.client_id)} className="w-full px-3 py-2 text-sm font-semibold bg-primary-action/20 text-brand-accent hover:bg-primary-action/30 rounded-md">+ Plan Campaign</button></div></InfoCard>
            )}
            <InfoCard title="Properties">
              <ul className="space-y-4">
                {properties.slice(0, 3).map(property => (
                  <li key={property.id} className="flex items-center gap-4">
                    <div className="relative w-20 h-16 bg-brand-dark rounded-md overflow-hidden"><Image src={property.image_urls?.[0] || `https://placehold.co/300x200/0B112B/C4C4C4?text=${property.address.split(',')[0]}`} alt={`Image of ${property.address}`} layout="fill" objectFit="cover" /></div>
                    <div><h4 className="font-semibold">{property.address}</h4><p>${property.price.toLocaleString()}</p><p className="text-xs text-brand-accent">{property.status}</p></div>
                  </li>
                ))}
              </ul>
            </InfoCard>
          </>
        ) : (<div className="flex flex-col items-center justify-center h-full text-center text-brand-text-muted"><Users className="w-16 h-16 mb-4" /><p>No Client Selected</p></div>)}
      </aside>
    </div>
  );
}