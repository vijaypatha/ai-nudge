...
def test_campaignbriefing_table_structure(session: Session, test_user: User):
    ...
    test_briefing = CampaignBriefing(
        id=uuid4(),
        user_id=test_user.id,  # <-- FIX: Use the ID from the existing test_user
    ...
    # The explicit cleanup (session.delete/commit) is no longer needed
    # because the session fixture now rolls back all changes automatically.
