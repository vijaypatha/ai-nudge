...
def upgrade() -> None:
    """Upgrade schema."""
    # Create enum types with checkfirst=True to avoid duplicate errors
    usertype_enum = postgresql.ENUM('REALTOR', 'THERAPIST', 'LOAN_OFFICER', name='usertype')
    usertype_enum.create(op.get_bind(), checkfirst=True)
    
    resourcestatus_enum = postgresql.ENUM('ACTIVE', 'INACTIVE', 'ARCHIVED', name='resourcestatus')
    resourcestatus_enum.create(op.get_bind(), checkfirst=True)
    
    resourcetype_enum = postgresql.ENUM('PROPERTY', 'WEB_CONTENT', 'CONTENT_RESOURCE', name='resourcetype')
    ...
    messagestatus_enum = postgresql.ENUM('PENDING', 'SENT', 'FAILED', 'CANCELLED', 'RECEIVED', name='messagestatus')
    messagestatus_enum.create(op.get_bind(), checkfirst=True)
    
    messagesource_enum = postgresql.ENUM('MANUAL', 'SCHEDULED', 'FAQ_AUTO_RESPONSE', 'INSTANT_NUDGE', name='messagesource')
    messagesource_enum.create(op.get_bind(), checkfirst=True)
    
    messagesendertype_enum = postgresql.ENUM('USER', 'AI', 'SYSTEM', name='messagesendertype')
    messagesendertype_enum.create(op.get_bind(), checkfirst=True)
    
    campaignstatus_enum = postgresql.ENUM('DRAFT', 'ACTIVE', 'PAUSED', 'COMPLETED', 'CANCELLED', name='campaignstatus')
    campaignstatus_enum.create(op.get_bind(), checkfirst=True)
    
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('user',
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('user_type', postgresql.ENUM('REALTOR', 'THERAPIST', 'LOAN_OFFICER', name='usertype', create_type=False), nullable=True),
    ...
    sa.Column('status', postgresql.ENUM('ACTIVE', 'INACTIVE', 'ARCHIVED', name='resourcestatus', create_type=False), nullable=False),
    ...
    sa.Column('id', sa.Uuid(), nullable=False),
    sa.Column('user_id', sa.Uuid(), nullable=False),
    sa.Column('resource_type', postgresql.ENUM('PROPERTY', 'WEB_CONTENT', 'CONTENT_RESOURCE', name='resourcetype', create_type=False), nullable=False),
    sa.Column('status', postgresql.ENUM('ACTIVE', 'INACTIVE', 'ARCHIVED', name='resourcestatus', create_type=False), nullable=False),
    ...
    sa.Column('status', postgresql.ENUM('PENDING', 'SENT', 'FAILED', 'CANCELLED', 'RECEIVED', name='messagestatus', create_type=False), nullable=False),
    sa.Column('source', postgresql.ENUM('MANUAL', 'SCHEDULED', 'FAQ_AUTO_RESPONSE', 'INSTANT_NUDGE', name='messagesource', create_type=False), nullable=False),
    sa.Column('sender_type', postgresql.ENUM('USER', 'AI', 'SYSTEM', name='messagesendertype', create_type=False), nullable=False),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    ...
    sa.Column('status', postgresql.ENUM('DRAFT', 'ACTIVE', 'PAUSED', 'COMPLETED', 'CANCELLED', name='campaignstatus', create_type=False), nullable=False),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['client_id'], ['client.id'], ),
...
def downgrade() -> None:
    ...
    # Drop enum types with checkfirst=True (only if no tables are using them)
